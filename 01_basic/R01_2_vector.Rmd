...
符號`<-` 將右邊的算式或數值指派給左邊的變數。右邊如果是numeric，那左邊的變數就是numeric variable；右邊如果是character，左邊的變數就是character variable。R的`<-`符號和一般程式語言的`=`意義是一樣的，甚至R也可以用`=`來作Assignment。在幾乎所有程式語言中，單等號`=`指的是Assignment，把右方的算式、值或物件指給左方的變數。而比較兩者相不相等，則用雙等號`==`，例如`1==3-2`。
